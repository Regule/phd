...
def old_main(csv_file, epoch_column, bias_column):
    ...
    x_tr = timeframe_shifter.fit_transform(x)
    y_tr = diff.fit_transform(y)
    x_tr = x_tr[1:]
    y_tr = mean_shifter.fit_transform(y_tr)
    y_tr = scaler.fit_transform(y_tr)
    x_tr = x_tr.reshape(-1,1)
    y_tr = y_tr.reshape(-1,1)

    print(f'After forward transform raw = {x.shape} transformed = {x_tr.shape}')

    x_tr = x_tr.flatten()
    y_tr = y_tr.flatten()
    y_tr = scaler.reverse_transform(y_tr)
    y_tr = mean_shifter.reverse_transform(y_tr)
    x_tr = np.insert(x_tr, 0, 0, axis=0)
    y_tr = diff.reverse_transform(y_tr)
    x_tr = timeframe_shifter.reverse_transform(x_tr)

    print(f'After reverse transform raw = {x.shape} transformed = {x_tr.shape}')
    print(f'RAW {x[0]}  TRANSFORMED {x_tr[0]}')
    
    if np.array_equal(x, x_tr) and np.array_equal(y, y_tr):
        print('All works just fine')
    else:
        plt.plot(x, y, color='green', label='Raw data')
        plt.plot(x_tr, y_tr, color='red', label='Transformed data')
        plt.title(f'Comparition between raw and transformed data')
        plt.xlabel('Time')
        plt.ylabel('Bias')
        plt.legend()
        plt.show()
...
def main(csv_file, json_file, epoch_column, bias_column):
    clock_data = pd.read_csv(csv_file, sep=';')
    clock_data[epoch_column] = clock_data[epoch_column].map(epoch_str_to_timestamp)
    x = clock_data.iloc[:][epoch_column].values
    y = clock_data.iloc[:][bias_column].values
    ...
    x_tr, y_tr = preprocessor.fit_transform(x, y)
    preprocessor.to_json(json_file)

    other_preprocessor = DataPreprocessor.load_json(json_file)
    other_preprocessor.fit_differentiator(y) # This have to be done for every new data
    print(preprocessor)
    print(other_preprocessor)
    x_tr, y_tr = other_preprocessor.reverse_transform(x_tr, y_tr)
...
def parse_arguments():
    ...
    parser.add_argument('-j', '--json_file',
                        help='file to which preprocessor data will be stored',
                        type=str,
                        required=True)
...
if __name__ == '__main__':
    ...
    main(args.csv_file, args.json_file, args.epoch_column, args.bias_column)
