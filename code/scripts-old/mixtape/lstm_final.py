...
#===========================================================================
def main(training_file, initializer_file, prediction_depth, window_size, metadata_folder,
         output_file, bias_column, epoch_column, validation_file, epochs):
    ...
    networks = build_network_models(x_tr.shape,None)
    for net_name, network in networks.items():
        history = network.fit(x_tr, y_tr, epochs=epochs, batch_size=32,
                              validation_data=(x_val, y_val), shuffle=False)
        model_json = network.to_json()
        file_path = build_output_file_path(metadata_folder, 'TMP_SAT', net_name, 'json')
        with open(file_path, "w") as json_file:
            json_file.write(model_json)
        file_path = build_output_file_path(metadata_folder, 'TMP_SAT', net_name, 'h5')
        network.save_weights(file_path)
        pd.DataFrame(history.history).to_csv(build_output_file_path(metadata_folder, 'TMP_SAT', f'{net_name}_history', 'csv'),
                                             sep=';', index=False)
...
def parse_arguments():
    ...
    parser.add_argument('--epochs',
                        ...
                        type=int,
                        default=10)
    parser.add_argument('-m', '--metadata_folder',
...
if __name__ == '__main__':
    ...
    main(args.training_file, args.initializer_file, args.prediction_depth, args.window_size, args.metadata_folder,
         args.output_file, args.bias_column, args.epoch_column, args.validation_file, args.epochs)
